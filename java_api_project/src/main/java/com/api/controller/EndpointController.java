...
public class EndpointController {
	...
	public ResponseEntity<Map<String, Integer>> getCounts() {
	    int totalEndpoints = endpointService.getTotalEndpoints();
	    int totalRequests = endpointService.getTotalRequests();
	    int activeEndpoints = endpointService.getActiveEndpointsCount();
	    int activeRequests = endpointService.getActiveRequestsCount();
	    int deactivatedEndpoints = endpointService.getDeactivatedEndpointsCount();
	    int deactivatedRequests = endpointService.getDeactivatedRequestsCount();

	    Map<String, Integer> counts = new HashMap<>();
	    counts.put("totalEndpoints", totalEndpoints);
	    counts.put("totalRequests", totalRequests);
	    counts.put("activeEndpoints", activeEndpoints);
	    counts.put("activeRequests", activeRequests);
	    counts.put("deactivatedEndpoints", deactivatedEndpoints);
	    counts.put("deactivatedRequests", deactivatedRequests);
